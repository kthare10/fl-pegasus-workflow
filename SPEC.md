...
subwf_round_r:
    train_round_r_client_* -> aggregate_round_r
```
...
- Use `stage_out=True` only for final reports, metrics, plots, and selected checkpoints.
- Use `stage_out=False` for intermediate per-round checkpoints unless they are needed for analysis.
